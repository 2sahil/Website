...
let currentPage = 1;
let pageSize = calculatePageSize(); // Number of images per page based on screen size
...
// Function to calculate the page size based on the screen size
function calculatePageSize() {
  const width = window.innerWidth;

  if (width < 600) {
    return 2; // For small screens (e.g., mobile)
  } else if (width < 900) {
    return 4; // For medium screens (e.g., tablets)
  } else {
    return 10; // For large screens (e.g., desktops)
  }
}
// Adjust pageSize on window resize
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  function openFullScreenModal(imageSrc) {
      ...
      fullScreenModal.style.display = "flex"; // Show the full-screen modal
  }

  // Add click event to the modal image
  const modalImage = document.getElementById("modal-image"); // Ensure you have the correct ID
  if (modalImage) {
      modalImage.onclick = function() {
          openFullScreenModal(this.src);
      };
  }

  // Close full-screen modal
  const closeButton = document.getElementById("close-full-screen");
  if (closeButton) {
      closeButton.onclick = function() {
          document.getElementById("full-screen-modal").style.display = "none"; // Hide the full-screen modal
      };
  }
  ...
  if (fullScreenModal) {
      fullScreenModal.onclick = function(event) {
          if (event.target === this) {
              this.style.display = "none"; // Hide the full-screen modal
          }
      };
  }
});
